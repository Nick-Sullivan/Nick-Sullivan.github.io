...
There exists three dice where A rolls higher than B more than 50% of the time, B rolls
higher than C more than 50% of the time, and C rolls higher than A more than 50% of the time.
This is known as [Intransitive Dice](https://en.m.wikipedia.org/wiki/Intransitive_dice).

For example, here are three 3-sided dice with this property, which you can modify to see
...
I approached this as an optimisation problem, with my objective function
measuring differences in win percentages. If my "water" dice only has a 51% win rate
...
To minimise ties (and to be frank, to make the code easier to write), I opted to not
...
The first case has bigger differences, but the second case is easier to follow because there are
